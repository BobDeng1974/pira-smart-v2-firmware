 ...
 *          It is set everytime when we enter state.
 *      stateTimeoutStart
 *          Set everytime we call state_transition funtion.
 */
state_e status_pira_state_machine = WAIT_STATUS_ON;
...
{
    uart_command_send('t', (uint32_t)settings_packet.data.status_time);
...
void state_transition(state_e next)
{
  ...
  statePrev=status_pira_state_machine;
 ...
 * @return char*
 */
char* return_state(state_e status_pira_state_machine)
{
...
void pira_state_machine()
{
    ...
    raspiSerial.print(return_state(statePrev));
    raspiSerial.print(" -> ");
